...
@content.each_key do |key|
    ...
    if @content[key].has_key? "perference"
      @content[key]["perference"].each_key do |cmd|
        commands.insert(-1, "git pull")
        commands.insert(-1, @content[key]["perference"][cmd])
        puts commands
    ...
    if @content[key].has_key? "account"
      @content[key]["account"].each_key do |cmd|
        commands.insert(-1, "git pull")
        commands.insert(-1, @content[key]["account"][cmd])
        kex.run_cmd(commands)
        commands.clear
        kex.clear_server
      end
