...
def getIPbyname(name)
  begin
    Socket::getaddrinfo(name, nil, 'AF_INET', Socket::SOCK_STREAM).each do |adr|
     return adr[3] if adr[3].include? "10."
    end
  rescue Exception => e
    return nil
  end
...
#customer your config here
    if @content[key].has_key? "board"
      @content[key]["board"].each_key do |cmd|
        commands.insert(-1, "git pull")
        commands.insert(-1, "bundle update")
        commands.insert(-1, @content[key]["board"][cmd])
        puts commands
